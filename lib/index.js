        ...
        _.each(rules, function (rule) {
            if (typeof rule === 'string') {
                ...
                _.each(rule, function (value, key) {
                    if (key.slice(-1) === '?') key = key.substr(0, key.length - 1);
                    var param = params[key];
                    if (param === undefined) {
                      // check for default param
                      value = _.isPlainObject(value) ? [value] : value;
                      if (_.isArray(value)) {
                        value.some((item) => {
                          if(_.isPlainObject(item) && 'default' in item) {
                            param = item['default'];
                            if (value.length === 1) {
                              value = 'any';
                            }
                    ...
                    else if (key.slice(-1) === '?' && params[key.substr(0, key.length - 1)] === undefined) return null;
