  ...
  if(typeof validations === 'string') {
    ...
    _.each(validations, function(validation){
      if (typeof validation === 'string' ) {
        ...
        // must be enum
        var enumCheck = checkEnumValidation(value, validation, key);
        if (valid.error) error.push(valid.error); else value = valid.value;
      }
    });
...
var checkEnumValidation = function(value, validation, key){
  if(_.isPlainObject(validations)) {
    var key = _.keys(validations)[0];
    var val = _.values(validations)[0];
    if (key === 'enum' && _.isArray(val)) {
      if (_.contains(validations, key)) {
        return {value: val};
      }
      return {error: 'The pvalue must be one of ' + validations.toString()};
    }
  }
  return {error: 'Error checking validations, valid validations: String , enum object, or Array ' +
    ' valid formats example: \n' +
